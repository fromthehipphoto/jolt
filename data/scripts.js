...
const MORNING_FIRST = [
  "Day 1. {{session}} on the board. Show up. Form before load. Begin when you're ready.",
  "First day. {{session}}. Short session, no heroics. I'll be here when you're back.",
];
const MORNING = [
  "Day {{day}}. {{session}} today. {{note}}",
  "Good morning. {{prevDays}} sessions in. {{session}} on the board. Begin when you're ready.",
...
export function morningMsg(vars, seed) {
  const pool = (vars.day === 1) ? MORNING_FIRST : MORNING;
